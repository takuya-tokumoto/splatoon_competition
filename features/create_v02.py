...
# A, Bチームのブキカテゴリーを指定してカウント（今回はチャージャーを指定）
...
@cached_feature("ft_charger_count_train")
def create_train_charger_count(train, test):
    print("prepare ft_charger_count_train") 
    
    ft_charger_count_train = buki_cate_count_agg(train, "charger")
    ft_charger_count_test  = buki_cate_count_agg(test,  "charger")

    return ft_charger_count_train

@cached_feature("ft_charger_count_test")
def create_test_charger_count(train, test):
    print("prepare ft_charger_count_train") 
    
    ft_charger_count_train = buki_cate_count_agg(train, "charger")
    ft_charger_count_test  = buki_cate_count_agg(test,  "charger")

    return ft_charger_count_test



@cached_feature("ft_armor_count_train")
def create_train_armor_count(train, test):
    print("prepare ft_armor_count_train") 
    
    ft_armor_count_train = special_cate_count_agg(train, "armor")
    ft_armor_count_test  = special_cate_count_agg(test,  "armor")

    return ft_armor_count_train

@cached_feature("ft_armor_count_test")
def create_test_armor_count(train, test):
    print("prepare ft_armor_count_test") 
    
    ft_armor_count_train = special_cate_count_agg(train, "armor")
    ft_armor_count_test  = special_cate_count_agg(test,  "armor")

    return ft_armor_count_test


@cached_feature("ft_ultrahanko_count_train")
def create_train_ultrahanko_count(train, test):
    print("prepare ft_ultrahanko_count_train") 
    
    ft_ultrahanko_count_train = special_cate_count_agg(train, "ultrahanko")
    ft_ultrahanko_count_test  = special_cate_count_agg(test,  "ultrahanko")

    return ft_ultrahanko_count_train

@cached_feature("ft_ultrahanko_count_test")
def create_test_ultrahanko_count(train, test):
    print("prepare ft_ultrahanko_count_test") 
    
    ft_ultrahanko_count_train = special_cate_count_agg(train, "ultrahanko")
    ft_ultrahanko_count_test  = special_cate_count_agg(test,  "ultrahanko")

    return ft_ultrahanko_count_test


@cached_feature("ft_nicedama_count_train")
def create_train_nicedama_count(train, test):
    print("prepare ft_nicedama_count_train") 
    
    ft_nicedama_count_train = special_cate_count_agg(train, "nicedama")
    ft_nicedama_count_test  = special_cate_count_agg(test,  "nicedama")

    return ft_nicedama_count_train

@cached_feature("ft_nicedama_count_test")
def create_test_nicedama_count(train, test):
    print("prepare ft_nicedama_count_test") 
    
    ft_nicedama_count_train = special_cate_count_agg(train, "nicedama")
    ft_nicedama_count_test  = special_cate_count_agg(test,  "nicedama")

    return ft_nicedama_count_test
...
if __name__ == "__main__":
    ...
    create_train_charger_count(in_train, in_test)
    create_test_charger_count(in_train, in_test)
    
    create_train_armor_count(in_train, in_test)
    create_test_armor_count(in_train, in_test)
    
    create_train_ultrahanko_count(in_train, in_test)
    create_test_ultrahanko_count(in_train, in_test)
    
    create_train_nicedama_count(in_train, in_test)
    create_test_nicedama_count(in_train, in_test)
