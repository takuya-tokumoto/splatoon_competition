...
def team_level_agg(input_data):
    ...
    output_data["rank_mean_diff"] = input_data[A_rank_col].mean(axis = 1) - input_data[B_rank_col].mean(axis = 1)
    ...
    output_data["level_mean_diff"] = input_data[A_level_col].mean(axis = 1) -  input_data[B_level_col].mean(axis = 1)
...
# A, Bチームのサブを指定してカウント
def subweapon_cate_count_agg(input_data, buki_cate):
    A_weapon_col = ["A1-weapon", "A2-weapon", "A3-weapon", "A4-weapon"]
    B_weapon_col = ["B1-weapon", "B2-weapon", "B3-weapon", "B4-weapon"]
    buki_col = A_weapon_col + B_weapon_col
     
    buki_data = pd.read_csv('./data/input/statink-weapon2.csv')
    target_buki_list    = list(buki_data[buki_data["subweapon"] == buki_cate]["key"].unique())
    no_target_buki_list = list(buki_data[buki_data["subweapon"] != buki_cate]["key"].unique())
...
@cached_feature("ft_all_subweapon_cate_count_train")
def create_ft_all_subweapon_count_train(train, test):
    print("prepare create_ft_all_subweapon_count_train") 
    
    buki_data = pd.read_csv('./data/input/statink-weapon2.csv')
    subweapon_cate_list = list(buki_data["subweapon"].unique())

    train_base = pd.DataFrame()
    test_base  = pd.DataFrame()

    for subweapon_cate in subweapon_cate_list:
        train_tmp = pd.DataFrame()
        test_tmp  = pd.DataFrame()

        train_tmp = subweapon_cate_count_agg(train, subweapon_cate)
        test_tmp  = subweapon_cate_count_agg(test,  subweapon_cate)
    ...
    ft_all_subweapon_cate_count_train = train_base
    ft_all_subweapon_cate_count_test  = test_base    

    return ft_all_subweapon_cate_count_train


@cached_feature("ft_all_subweapon_cate_count_test")
def create_ft_all_subweapon_count_test(train, test):
    print("prepare create_ft_all_subweapon_count_test") 
    ...
    return ft_all_subweapon_cate_count_test
...
if __name__ == "__main__":
    ...
    create_ft_all_subweapon_count_train(in_train, in_test) 
    create_ft_all_subweapon_count_test(in_train, in_test)
